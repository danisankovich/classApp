app.controller('mailCtrl', function($scope, $state, $http){
  ...
  $http.get('localhost:3000/user').success(function(user) {
    $scope.user = user;
    $http.get('localhost:3000/mail/mymail').success(function(messages) {
        messages.forEach(function(e) {
          if (e.recipientId === $scope.user._id) {
            $http.get("localhost:3000/mail/sender/" + e.senderId).success(function(sender) {
      ...
      user.friends.forEach(function(friend) {
        $http.get('localhost:3000/friends/' + friend.friendId, friend).success(function(friend) {
  ...
  $scope.sendMail = function(message) {
    ...
    $http.post("localhost:3000/mail/send/" + thisUserId, message).success(function(sentMessage) {
...
app.controller('oneMsgCtrl', function($scope, $state, $http){
  ...
  $http.get('localhost:3000/mail/onemessage/' + $state.params.msgId).success(function(message) {
    console.log(message);
    $http.get("localhost:3000/mail/sender/" + message.senderId).success(function(sender) {
  ...
  $scope.replyUser = function(message) {
    console.log("thisuserid", thisUserId);
    console.log("mymessage", message);
    $http.post("localhost:3000/mail/reply/" + thisUserId, message).success(function(sentMessage) {
      console.log(sentMessage);
      swal("Message Sent!", "Your message has been sent and will be delivered shortly!", "success");
    });
  };
